...
if __name__ == '__main__':
    ...
    test_img_ids, val_img_ids = train_test_split(val_img_ids, test_size=0.5, random_state=random.randint(0, 1024))

    with open(os.path.join(root, '{}_train.txt'.format(name)), 'w') as file:
        for i in train_img_ids:
            file.write(i + '\n')
    print("built train file successfully, path is: {}".format(os.path.join(root, '{}_train.txt'.format(name))))

    with open(os.path.join(root, '{}_val.txt'.format(name)), 'w') as file:
        for i in val_img_ids:
            file.writelines(i + '\n')
    print("built validate file successfully, path is: {}".format(os.path.join(root, '{}_val.txt'.format(name))))
    with open(os.path.join(root, '{}_test.txt'.format(name)), 'w') as file:
        for i in test_img_ids:
            file.writelines(i + '\n')
    print("built test file successfully, path is: {}".format(os.path.join(root, '{}_test.txt'.format(name))))
